...
public class KinoaSyncGameEventsService : KinoaGameEventBuildingService<KinoaSyncGameEventsService>
{
    ...
    public void ProcessResponse(Response<SyncGameEventResponse> response)
    {
        ...
        ProcessMissionsProgressInApps(response.Data);
	...
	/// <summary>
    ///     Process the Inbox Missions Progress In-apps from Synchronous Game Event sending response.
    /// </summary>
    private void ProcessMissionsProgressInApps(SyncGameEventResponse response)
    {
        var missionsProgressInApps = response.InboxMessages
            .Where(msg => response.InboxDetails.MissionProgressionInApps.Contains(msg.Uuid))
            .Select(msg => msg)
            .ToArray();
        ...
        if (missionsProgressInApps.Any())
        {
            KinoaUiService.Instance.ReplaceGameInApps(missionsProgressInApps,
                nameof(SyncGameEventResponse), nameof(missionsProgressInApps), addToDisplayQueue: false);
        }
    }
