...
public class KinoaPlayerStateService : KinoaSingleton<KinoaPlayerStateService>
{
    ...
    /// <summary>
    ///     Returns the local Player State at session-open. Local state is the source of truth
    ///     (offline-resilient — server replicates client mutations).
    /// </summary>
    /// <remarks>
    ///     <para>
    ///     <b>Recommended pattern:</b> populate <see cref="CustomPlayerState"/> from your save / data classes via
    ///     a parameterized constructor — return <c>new CustomPlayerState(&lt;sources&gt;)</c>. For
    ///     async sources (cloud save, file I/O), use an async <c>PopulateAsync</c> instance method
    ///     instead (constructors can't <c>await</c>). Keep <see cref="CustomPlayerState"/> narrow —
    ///     Kinoa-Dashboard-relevant fields only; see its XML doc for selection guidance.
    ///     </para>
    ///     <para>
    ///     <b>Null is acceptable ONLY</b> when the game has no local state (truly fresh first install,
    ///     no prior save) — the server returns the existing player (matched by <c>Kinoa.Player.ID</c>)
    ///     or inits a new one. Do NOT return null when local state exists; on a session start with
    ///     offline-queued events that produces client/server divergence.
    ///     </para>
    /// </remarks>
    private async Task<CustomPlayerState> GetLocalPlayerStateAsync()
    {
        //TODO: Populate from your local storage. See XML doc for constructor / PopulateAsync pattern;
        //      see CustomPlayerState for field-selection guidance.
