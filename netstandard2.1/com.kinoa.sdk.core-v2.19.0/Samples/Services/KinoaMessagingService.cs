...
using Kinoa.Data.Messaging.InApp.Features.Missions;
...
public class KinoaMessagingService : KinoaSingleton<KinoaMessagingService>
{
    ...
    private void ProcessInboxInApps(IEnumerable<InAppMessage> messages)
    {
		...
		ProcessMissionsProgressInApps(inboxMessages);
	...
	/// <summary>
    ///     Processes the missions progress Inbox In-app messages received via WebSocket.
    /// </summary>
    /// <param name="messages">The In-app messages.</param>
    private void ProcessMissionsProgressInApps(IEnumerable<InAppMessage> messages)
    {
        var missionsProgressInApps = messages
            .Where(x => x.Command is InAppMissionsProgressChangedCommand)
            .ToList();

        if (missionsProgressInApps.Any())
        {
            KinoaUiService.Instance.ReplaceGameInApps(missionsProgressInApps,
                "WebSocket", nameof(InAppMilestonesProgressChangedCommand), addToDisplayQueue: false);
        }
    }
    ...
    /// <summary>
    ///     Gets only unviewed inbox In-app messages (those not yet acknowledged as viewed).
    /// </summary>
    public async Task<Response<List<InAppMessage>>> GetUnviewedInboxMessagesAsync()
    {
        var response = await Kinoa.Messaging.GetInboxMessagesAsync(isViewed: false);
        if (!response.IsSuccessful() || response.Data == null)
        {
            Log($"{nameof(GetUnviewedInboxMessagesAsync)} status: {response.Status}.");
            return response;
        }

        //TODO: Replace with your game's UI logic.
        // Tip: use response.Data.Count for a lobby "unread" badge (skip the call below).
        KinoaUiService.Instance.ReplaceGameInApps(response.Data,
            nameof(response), nameof(GetUnviewedInboxMessagesAsync), addToDisplayQueue: false);

        return response;
    }

    /// <summary>
    ///     Acknowledges a single inbox In-app message as viewed.
    ///     Typical trigger: when the in-app is actually shown to the player (e.g., impression event).
    /// </summary>
    /// <remarks>
    ///     For tap / impression handlers where UI lag is unacceptable, use the optimistic UI variant
    ///     <see cref="AcknowledgeInboxMessageViewOptimisticallyAsync"/> instead.
    /// </remarks>
    public async Task<Response<InAppAcknowledgeViewResult>> AcknowledgeInboxMessageViewAsync(InAppMessage message)
    {
        if (message == null) return null;

        var isViewedBefore = message.IsViewed;
        var response = await Kinoa.Messaging.AcknowledgeInboxMessageViewAsync(message);
        var acknowledgedUuids = response.Data?.AcknowledgedUuids ?? new List<string>();

        Log($"{nameof(AcknowledgeInboxMessageViewAsync)} status: {response.Status}. " +
            $"Requested: '{message.Name}' '{message.Uuid}'. " +
            $"Acknowledged ({acknowledgedUuids.Count}): {string.Join(", ", acknowledgedUuids)}. " +
            $"IsViewed: {isViewedBefore} → {message.IsViewed}.");

        if (!response.IsSuccessful()) return response;

        //TODO: Replace with your game's UI logic.
        // Hide the "new" badge on the corresponding UI element.

        return response;
    }

    /// <summary>
    ///     Acknowledges a single inbox In-app message as viewed — optimistic UI variant.
    ///     Flips <see cref="InAppMessage.IsViewed"/> locally immediately, then dispatches the server
    ///     sync in the background. Best for tap / impression handlers where UI lag is unacceptable.
    ///     Failures self-heal on the next <see cref="GetInboxMessagesAsync"/>.
    /// </summary>
    public Task<Response<InAppAcknowledgeViewResult>> AcknowledgeInboxMessageViewOptimisticallyAsync(InAppMessage message)
    {
        if (message == null) return Task.FromResult<Response<InAppAcknowledgeViewResult>>(null);
        ...
        Log($"{nameof(AcknowledgeInboxMessageViewOptimisticallyAsync)} fired (optimistic). " +
            $"In-app: '{message.Name}' '{message.Uuid}'. " +
            $"IsViewed: {isViewedBefore} → {message.IsViewed}.");

        return Kinoa.Messaging.AcknowledgeInboxMessageViewAsync(message);
    }

    /// <summary>
    ///     Acknowledges multiple inbox In-app messages as viewed in a single request.
    ///     Typical trigger: closing an inbox UI panel where the player saw a batch of in-apps at once.
    /// </summary>
    public async Task<Response<InAppAcknowledgeViewResult>> AcknowledgeInboxMessagesViewAsync(
        IEnumerable<InAppMessage> messages)
    {
        var inApps = messages?.ToList() ?? new List<InAppMessage>();
        var response = await Kinoa.Messaging.AcknowledgeInboxMessagesViewAsync(inApps);
        var acknowledgedUuids = response.Data?.AcknowledgedUuids ?? new List<string>();
        var viewedCount = inApps.Count(m => m?.IsViewed == true);

        Log($"{nameof(AcknowledgeInboxMessagesViewAsync)} status: {response.Status}. " +
            $"Requested ({inApps.Count}): {FormatInApps(inApps)}. " +
            $"Acknowledged ({acknowledgedUuids.Count}): {string.Join(", ", acknowledgedUuids)}. " +
            $"IsViewed=true count: {viewedCount}/{inApps.Count}.");

        if (!response.IsSuccessful()) return response;

        //TODO: Replace with your game's UI logic.
        // Hide the "new" badges on the corresponding UI elements.

        return response;
    }

    private static string FormatInApps(IEnumerable<InAppMessage> inApps)
        => string.Join(", ", inApps.Select(m => $"'{m?.Name}' '{m?.Uuid}'"));
    ...
    /// <summary>
    ///     Collects missions progress for inbox In-app message.
    /// </summary>
    public async Task<List<InAppMissionsProgressResult>> CollectMissionProgressOfInAppAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var response = await Kinoa.Messaging.CollectMissionsProgress(inAppMessage,  setNumber, rowNumbers, cancellationToken);

        if (response.IsSuccessful() && response.Data != null)
        {
            logBuilder.Append($"{nameof(CollectMissionProgressOfInAppAsync)} for In-app message '{inAppMessage.Name}' '{inAppMessage.Uuid}':");
            return response.Data;
        }
        
        logBuilder.Append($"{nameof(CollectMissionProgressOfInAppAsync)} request status: {response.Status.ToString()}.");
        Debug.Log(logBuilder.ToString());
        return null;
    }
    
    /// <summary>
    ///     Reaches missions progress for inbox In-app message.
    /// </summary>
    public async Task<InAppMessage> ReachMissionProgressOfInAppAsync(
        InAppMessage inAppMessage,
        int setNumber,
        List<int> rowNumbers,
        CancellationToken cancellationToken = default)
    {
        var logBuilder = new StringBuilder();
        var response = await Kinoa.Messaging.ReachMissionsProgress(inAppMessage,  setNumber, rowNumbers, cancellationToken);

        if (response.IsSuccessful() && response.Data != null)
        {
            logBuilder.Append($"{nameof(ReachMissionProgressOfInAppAsync)} for In-app message '{inAppMessage.Name}' '{inAppMessage.Uuid}':");
            return response.Data;
        }
        
        logBuilder.Append($"{nameof(ReachMissionProgressOfInAppAsync)} request status: {response.Status.ToString()}.");
        Debug.Log(logBuilder.ToString());
        return null;
    }
    
    /// <summary>
    ///     Collect missions milestones progress for inbox In-app message.
    /// </summary>
    public async Task<List<InAppMissionsMilestonesResult>> CollectMissionsMilestonesProgressOfInAppAsync(
        InAppMessage inAppMessage,
        int setNumber,
        List<int> rowNumbers,
        CancellationToken cancellationToken = default)
    {
        var logBuilder = new StringBuilder();
        var response = await Kinoa.Messaging.CollectMissionsMilestonesProgress(inAppMessage,  setNumber, rowNumbers, cancellationToken);

        if (response.IsSuccessful() && response.Data != null)
        {
            logBuilder.Append($"{nameof(CollectMissionsMilestonesProgressOfInAppAsync)} for In-app message '{inAppMessage.Name}' '{inAppMessage.Uuid}':");
            return response.Data;
        }
        
        logBuilder.Append($"{nameof(CollectMissionsMilestonesProgressOfInAppAsync)} request status: {response.Status.ToString()}.");
        Debug.Log(logBuilder.ToString());
        return null;
    }
    
    /// <summary>
    ///     Deletes missions progress for inbox In-app message.
    /// </summary>
    public async Task DeleteMissionProgressOfInAppAsync(
        InAppMessage inAppMessage,
        int setNumber,
        List<int> rowNumbers,
        CancellationToken cancellationToken = default)
    {
        var logBuilder = new StringBuilder();
        var response = await Kinoa.Messaging.DeleteMissionsProgress(inAppMessage,  setNumber, rowNumbers, cancellationToken);

        if (response.IsSuccessful())
        {
            logBuilder.Append($"{nameof(DeleteMissionProgressOfInAppAsync)} for In-app message '{inAppMessage.Name}' '{inAppMessage.Uuid}':");
            
        }
        else
        {
            logBuilder.Append($"{nameof(DeleteMissionProgressOfInAppAsync)} request status: {response.Status.ToString()}.");
        }
    ...
    private void LogInAppDetails(InAppMessage inApp)
    {
        ...
        sb.AppendLine($"IsViewed: {inApp.IsViewed}");
        ...
        // Feature Configuration Mode: Define manually
        if (inApp.FeatureConfigurations?.Any() == true)
        {
            sb.AppendLine($"FeatureConfigurations ({inApp.FeatureConfigurations.Count}):");
        }

        // Feature Configuration Mode: Use existing Feature Settings
        if (inApp.FeatureSettings?.Any() == true)
        {
            sb.AppendLine($"FeatureSettings ({inApp.FeatureSettings.Count}):");
            foreach (var fs in inApp.FeatureSettings)
            {
                // Filters = player-state values the server used to pick the matched row of this FS.
                var filters = fs.Filters?.Any() == true
                    ? string.Join(", ", fs.Filters.Select(kv => $"{kv.Key}={kv.Value}"))
                    : "(none)";
                sb.AppendLine($"\tKey={fs.Key}, ConfigurationName={fs.ConfigurationName}, Data={fs.Data?.Count ?? 0}, Filters=[{filters}]");
            }

            // Pick the first FS key just for the demos below.
            var firstKey = inApp.FeatureSettings[0].Key;

            // Lookup a single Feature Setting by key — when ConfigurationName / metadata is needed.
            var firstFs = inApp.GetFeatureSetting(firstKey);
            sb.AppendLine($"\tGetFeatureSetting(\"{firstKey}\") → ConfigurationName={firstFs?.ConfigurationName ?? "null"}");

            // Typed configs from one specific FS — when multiple FS share the same schema and must be disambiguated.
            var firstFsBonuses = inApp.GetFeatureConfigurations<InAppDailyBonusFeatureConfiguration>(firstKey);
            sb.AppendLine($"\tGetFeatureConfigurations<DailyBonus>(\"{firstKey}\") → {firstFsBonuses.Count} config(s)");

            // List FS that carry configs of a given type — to iterate per-FS with metadata.
            var dailyBonusSettings = inApp.GetFeatureSettings<InAppDailyBonusFeatureConfiguration>();
            sb.AppendLine($"\tGetFeatureSettings<DailyBonus>() → {dailyBonusSettings.Count} setting(s)");
        }

        // Mode-agnostic accessor — pulls typed configs regardless of Feature Configuration Mode.
        var dailyBonuses = inApp.GetFeatureConfigurations<InAppDailyBonusFeatureConfiguration>();
        if (dailyBonuses.Any())
        {
            sb.AppendLine($"DailyBonus configurations ({dailyBonuses.Count}):");
            foreach (var bonus in dailyBonuses)
                // LevelFilter / LevelFilterFrom / LevelFilterTo = row's dashboard-configured filter criteria.
                sb.AppendLine($"\tCoins={bonus.Coins}, LevelFilter={bonus.LevelFilter}");
        }
    ...
    private static void LogClickConfig(StringBuilder sb, string prefix, InAppClickConfiguration click)
    {
        ...
        sb.AppendLine(click switch
        {
            ...
            InAppCustomClickConfiguration custom => $"{nameof(InAppCustomClickConfiguration)} " +
                $"(CtaName: {custom.CtaName})",
