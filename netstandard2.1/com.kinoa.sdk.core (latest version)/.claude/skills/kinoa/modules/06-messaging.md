...
1. **Existing popup / modal class fits — adapt directly.** Game has a class like `PopUp`, `Dialog`, `BaseUIGroup`, `IPopup`, etc. that already represents in-app modal UI. `KinoaUiService` stub bodies become thin adapters that:
   ...
   - Wire SDK-fired button-action types (CollectResource / ShowAd / Billing / Close / DeepLink — see `InAppCustomTemplateData.Buttons[i].ClickConfiguration`) to the game's existing handlers (IAP, ad SDK, resource grant, deep-link router, close).
...
- Click-action handlers per `InAppCustomTemplateData` button-action types: `CollectResource` (grant rewards), `ShowAd` (gate on rewarded-ad ready, then grant on completion), `Billing` (route to game's IAP service), `Close`, `DeepLink` (route to game's screen / scene / shop / level-map).
...
- `Kinoa.Messaging.GetInboxMessagesAsync()` — fetches all server-side inbox messages
...
- `InAppMessage.GetFeatureConfigurations<T>()` — returns typed feature configuration entries attached to the message
...
- **Feature Configurations:** Register types before `SDK.Initialize()` (see 01-init). Exact version match takes priority over version-agnostic. Unregistered types are deserialized as base `InAppFeatureConfiguration` with raw data in `ExtensionData` — no data is lost. Access at runtime via `message.GetFeatureConfigurations<T>()`.
